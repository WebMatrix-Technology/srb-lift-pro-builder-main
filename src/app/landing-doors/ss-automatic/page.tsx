...
import { Shield, ArrowRight, ArrowLeft, CheckCircle2, Sparkles, Building2 } from "lucide-react";
import Reveal from "@/components/Reveal";
import Link from "next/link";
import { Badge } from "@/components/ui/badge";
import heroElevatorImg from "@/assets/hero-elevator.jpg";

export default function SSAutomatic() {
  const features = [
  ...
  const specifications = [
    ...
    { label: "Finish", value: "Premium Sleek" },
    { label: "Maintenance", value: "Low Maintenance" },
    { label: "Durability", value: "High Corrosion Resistance" },
    { label: "Design", value: "Modern Aesthetic" },
  ];

  return (
    <div className="flex min-h-screen flex-col">
      <section className="relative overflow-hidden bg-gradient-to-br from-slate-600 via-slate-700 to-slate-800 py-28 md:py-36">
        ...
        <div className="container relative mx-auto px-4">
          <Reveal variant="fade-up">
            ...
            <h1 className="mb-6 text-5xl font-bold leading-tight text-white md:text-6xl lg:text-7xl">
              SS <span className="bg-gradient-to-r from-yellow-200 via-yellow-100 to-white bg-clip-text text-transparent">Automatic</span>
            </h1>
            ...
            <Button size="lg" className="bg-white text-slate-700 hover:bg-white/90 hover:shadow-xl transition-all duration-300" asChild>
          ...
          <div className="grid gap-8 lg:grid-cols-2">
            <Reveal variant="fade-up">
              <div className="relative aspect-video w-full overflow-hidden rounded-2xl">
                <img src={typeof heroElevatorImg === 'string' ? heroElevatorImg : heroElevatorImg.src} alt="SS Automatic Door" className="h-full w-full object-cover" />
              ...
              <Card className="h-full border-2 border-primary/20 bg-gradient-to-br from-background to-muted/20 shadow-xl">
                ...
                <CardContent className="space-y-6">
                  <p className="text-lg leading-relaxed text-muted-foreground">
                    SS Automatic doors feature premium stainless steel construction with sleek finish and smooth automatic operation. These doors are perfect for modern buildings that require premium aesthetics combined with reliable performance and low maintenance requirements.
                  </p>
              ...
              <Reveal key={index} variant="fade-up" delayMs={index * 100}>
                <Card className="group h-full border-2 border-border/50 bg-gradient-to-br from-background to-muted/20 transition-all duration-500 hover:border-primary/50 hover:shadow-xl hover:-translate-y-1">
                  <CardContent className="p-6">
                    <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-slate-500 to-slate-600 shadow-lg transition-transform duration-300 group-hover:scale-110">
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-slate-600 via-slate-700 to-slate-800 py-24">
        <div className="absolute inset-0 bg-grid-pattern opacity-10" />
        <div className="container relative mx-auto px-4 text-center">
          <Reveal variant="fade-up">
            <h2 className="mb-4 text-4xl font-bold text-white md:text-5xl">Need SS Automatic Doors?</h2>
            <p className="mb-10 text-xl text-white/95">Contact us for premium stainless steel door solutions</p>
            <Button size="lg" className="bg-white text-slate-700 hover:bg-white/90 hover:shadow-2xl hover:scale-105 transition-all duration-300 h-16 px-10 text-lg font-semibold" asChild>
              <Link href="/contact">
