...
import { Droplets, ArrowRight, ArrowLeft, CheckCircle2, Home, Building2, Zap } from "lucide-react";
import Reveal from "@/components/Reveal";
import Link from "next/link";
import { Badge } from "@/components/ui/badge";

export default function HydraulicLifts() {
  const images = [
    "/images/doors/hydrolic_lift_1.png",
    "/images/doors/hydrolic_lift_2.png",
  ];
  const features = [
  ...
  const specifications = [
    { label: "Capacity", value: "3-20 persons (204-1360 KG)" },
    { label: "Operation", value: "Hydraulic (Oil pressure)" },
    { label: "Power", value: "Single-phase" },
    { label: "Pit Requirement", value: "Not required" },
  ...
  return (
    <div className="flex min-h-screen flex-col">
      <section className="relative overflow-hidden bg-gradient-to-br from-green-600 via-emerald-600 to-green-700 py-28 md:py-36">
        ...
        <div className="container relative mx-auto px-4">
          <Reveal variant="fade-up">
            ...
            <h1 className="mb-6 text-5xl font-bold leading-tight text-white md:text-6xl lg:text-7xl">
              Hydraulic <span className="bg-gradient-to-r from-yellow-200 via-yellow-100 to-white bg-clip-text text-transparent">Lifts</span>
            </h1>
            <p className="mb-8 text-xl text-white/95 md:text-2xl">
              Operate using oil pressure to move a piston upwards and gravitational force for lowering. Ultra-smooth and quiet ride quality
            </p>
            <Button size="lg" className="bg-white text-green-600 hover:bg-white/90 hover:shadow-xl transition-all duration-300" asChild>
              <Link href="/products">
              ...
              <div className="space-y-4">
                {images.map((imageSrc, index) => (
                  <div key={index} className="relative aspect-video w-full overflow-hidden rounded-2xl">
                    <img src={imageSrc} alt={`Hydraulic Lift ${index + 1}`} className="h-full w-full object-cover" />
              ...
              <Card className="h-full border-2 border-primary/20 bg-gradient-to-br from-background to-muted/20 shadow-xl">
                ...
                <CardContent className="space-y-6">
                  <p className="text-lg leading-relaxed text-muted-foreground">
                    Hydraulic lifts operate using oil pressure to move a piston upwards and gravitational force for lowering. These lifts are ideal for low-rise buildings where space constraints or cost considerations make them the perfect choice. They offer ultra-smooth operation with minimal installation requirements.
                  </p>
              ...
              <Reveal key={index} variant="fade-up" delayMs={index * 100}>
                <Card className="group h-full border-2 border-border/50 bg-gradient-to-br from-background to-muted/20 transition-all duration-500 hover:border-primary/50 hover:shadow-xl hover:-translate-y-1">
                  <CardContent className="p-6">
                    <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-green-500 to-emerald-500 shadow-lg transition-transform duration-300 group-hover:scale-110">
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {applications.map((app, index) => {
              const icons = [Home, Building2, Building2, Home, Building2, Building2];
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-green-600 via-emerald-600 to-green-700 py-24">
        ...
        <div className="container relative mx-auto px-4 text-center">
          <Reveal variant="fade-up">
            <h2 className="mb-4 text-4xl font-bold text-white md:text-5xl">Need Hydraulic Lift Solutions?</h2>
            <p className="mb-10 text-xl text-white/95">Contact us for cost-effective lift solutions</p>
            <Button size="lg" className="bg-white text-green-600 hover:bg-white/90 hover:shadow-2xl hover:scale-105 transition-all duration-300 h-16 px-10 text-lg font-semibold" asChild>
              <Link href="/contact">
